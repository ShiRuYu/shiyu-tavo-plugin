...
- **角色管理**：按完整对话回合自动检测剧情中新角色，也可手动检测当前剧情，或根据描述生成可编辑候选卡；确认后创建 Tavo 角色卡。
- **内容导入**：输入 `owner/repo` 或 GitHub URL 与分支后按目录浏览；支持勾选、全选、预览和批量导入预设、世界书、正则、CCv2/CCv3/SillyTavern 角色卡，并对重复资源提供更新、新建副本或跳过。
- **微信**：每个手机身份拥有独立的联系人、会话和朋友圈；支持手动扫描当前剧情、生成后自动扫描、主动消息和角色间聊天。
...
3. 打开 Advanced Rendering，聊天页会出现可拖动的小手机入口；打开后可拖动顶部栏移动整部手机。
