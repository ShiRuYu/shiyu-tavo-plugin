...
test('automatic social scanning can be disabled without affecting manual calls', async () => {
  const threads = [];
  const state = {
    activeIdentityId: 'user:default',
    identities: [
  ...
  const coordinator = createRuntimeCoordinator({
    ...
    adapter: {},
    storage: { loadGlobal: async () => state, saveGlobal: async () => {}, loadChatLink: async () => ({ digest: '' }) },
    threadService: { getOrCreateThread: async (input) => { threads.push(input); return { id: 'thread-1', ...input }; } },
    roleDialogueService: { generateExchange: async () => {} },
  });
  const result = await coordinator.runBackground({ chatId: 7, text: '周澜不在现场。', at: '2026-09-16T00:00:00.000Z', presentCharacterIds: [] });
  assert.equal(result.proactive, 0);
